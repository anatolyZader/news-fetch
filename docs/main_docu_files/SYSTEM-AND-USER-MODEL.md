...
**Purpose:** Describe how the running product supports **human decision-making** — not automated verdicts. Users scan attention and **evidence-backed claims**; developers see the same report with additional score-revealing detail.
...
Srulik's lab ingests multi-source text (news, radio, WhatsApp, field reports, PBO, social OSINT, etc.), extracts closed-vocabulary behavioral signals, runs **assessment agent + RAG investigation** (default), and presents **user-safe** surfaces:
...
Shadow numeric scores remain in on-disk JSON under `daily_reports/` and are visible to developers in the report UI; the separate drift/divergence calibration tooling that used to consume them has been retired. API and user UI **redact** headline scores at the boundary.

---

## User loop: scan → proof → decide
...
3. **Decide** — Human judgment; optional user recommendations workflow (`UserRecommendationsPanel.jsx`, `/api/report/recommendations/*`). System may **abstain** (`insufficient_data`, `sampling_blind`, data void) — treat abstention as a prompt to ingest or wait, not “all clear.” When chat budget is exhausted during crisis epistemic conditions, report API may return `suggest_crisis_budget: true` ([COST-CONTROLS.md § Crisis chat budget](./COST-CONTROLS.md#crisis-chat-budget-cb-hybrid)). `shouldSuggestCrisisBudget` (`crisisBudgetService.js`) fires when daily/crisis budget is exceeded **and** (`data_void.level === 'critical'` OR `digital_darkness` OR `sampling_status === 'blind'` OR `assessment_mode === 'abstained'`).

**Crisis budget UI (where activation lives):**

| Role | Surface | Component |
|------|---------|-----------|
| **User** | User app `client/` | `EpistemicStatusBanner` shows `crisisBudget.userBanner` via `epistemicBannerMessages.js` when `suggest_crisis_budget` is true |
| **Developer** | User app `client/` | `CrisisBudgetPanel` in `MainApp.jsx`, gated by `canViewDeveloper` — HITL activate/deactivate crisis chat pool |

Developers open the same **user app** (same origin, same login) to activate the pool — there is no separate developer app.
...
|------|-----|-----|----------------|
| **User (default)** | `client/` → `MainApp.jsx` passes `displayTier="user"` to `ReportView` | `GET /api/report/today` default (`app/reportCacheService.js` → `getCachedReport`); `resolveDisplayView` → `user` | Narratives, **evidence_tree**, instruments, attention, **action_compass**, **anomaly_strip**, recommendations, optional `suggest_crisis_budget`. **No** headline 1–10 scores. |
| **Developer** | Same app `client/` → `ReportView.jsx`, `?view=developer` | Same report route when `canViewDeveloperDisplay(email)` | Score-revealing components shown inline in the same report view (`InstrumentMetricsBadges`, `WhyThisScore`, `DeltaLine`, `OovAnomalyClustersPanel`, `ReportComponentFilterBar`, `EvidenceOverviewPanel`, `NationalContextSection`, facet bars, etc.), plus `CrisisBudgetPanel` crisis-budget activation in `MainApp.jsx`. Scores still largely redacted at API for users. The separate `developer-site/` SPA (drift sparklines, validation review, catalog proposals, agent trace replay, pipeline status) has been retired. |

Access: `config/userAccess.json` levels `developer` / `maintainer`, or env `RESILIENCE_ANALYST_EMAILS` / `RESILIENCE_MAINTAINER_EMAILS` (`cross-cut-modules/auth/userAccess.js`).

Implementation: `redactReportPayload`, `redactAssessmentForView`, `deriveInstrumentState` in `assessmentDisplayTier.js`.

---

## What users see instead of scores
...
| `user_shows_score` | Rare exception when a critical single signal may show 1–10 (salience bypass) |
...
| `polarization_band`, `certainty_band` | Categorical bands safe for users |
...
**Report view** (`ReportView.jsx`, user tier):

1. Epistemic status banner
2. Evidence overview (instrument summary counts)
3. Attention panel
4. **Action compass** — ranked user actions from `buildActionCompass` in `actionCompass.js` (`action_compass` on report API); uncertainty band from `deriveUncertaintyBand` (data void, sampling, abstention); gap-closure tasks; **geo-unknown** warning when `geoUnknownReviewService` reports pending `new` queue items (resolution remains developer HITL — see [GEOGRAPHIC-ANALYSIS.md § Developer unknown queue](./GEOGRAPHIC-ANALYSIS.md#developer-unknown-queue))
...
6. User recommendations (when present)
...
**Developer view** — no separate header link or workspace; `displayView="developer"` (`?view=developer`) reveals score-carrying components inline within the same `ReportView.jsx` used by users. The separate `developer-site/` SPA (`DeveloperApp.jsx`, with its scope switcher, assessment/drift/pipeline tabs, validation/catalog panels, and agent-trace replay) has been retired.

---

## API redaction summary

`assessmentDisplayTier.js` strips from user payloads (non-exhaustive):
...
- Macro signals summarized to counts/types for users

Developer tier keeps additional instrument detail (`suppression_delta`, truncated `top_contributors`) but is not a “full score dashboard” in the default API shape.
