...
| Term | Meaning |
|------|---------|
| **User** | Default user. Sees narratives, evidence, instruments, attention — not headline 1–10 scores. |
| **Developer** | Privileged user (`canViewDeveloperDisplay`). Sees headline scores and score-revealing UI (`WhyThisScore`, `DeltaLine`, `InstrumentMetricsBadges`, etc.) that users do not; also gates `CrisisBudgetPanel` crisis-chat-budget activation. |
...
Access levels: `user` \| `developer` \| `maintainer` — `config/userAccess.json`, `cross-cut-modules/auth/userAccess.js`.
...
| **Instrument** | User-safe flag (sufficiency, contestation, salience) — not a hidden score. |
...
| **Thin evidence** | Low evidence mass; user instruments hide or qualify scores. |
...
|----------------|---------|----------------|
| **display_view** | `user` \| `developer` API/UI redaction | "display tier" |
...
- User model: [SYSTEM-AND-USER-MODEL.md](../main_docu_files/SYSTEM-AND-USER-MODEL.md)
