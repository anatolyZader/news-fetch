...
- **Fix**: ensure the deploy includes `docs/product_docs/`. If stub template pages surface, the index filter isn't excluding `_`-prefixed files; update `utils/productDocs.js` to match the validator's exclusions and redeploy.
