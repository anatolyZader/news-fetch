...
import { useChatbotManualReports } from '../hooks/useChatbotManualReports.js';
...
export function ChatbotManualReportsTab() {
  const theme = useTheme();
  const { t } = useLanguage();
  const { apiReady, getIdToken } = useAuth();
  const { data, loading, error } = useChatbotManualReports({
    getIdToken,
    apiReady,
  });
  ...
  const title = t('tab.chatbot');
  const subtitle = t('chatbotManual.subtitle');
  ...
  const loadFull = useCallback(
    async (fileName) => {
      ...
      try {
        ...
        const res = await fetch(`/api/chatbot/manual-reports/file?${q.toString()}`, { headers });
        ...
        setFullByName((prev) => ({
          ...prev,
          [fileName]: String(t('chatbotManual.loadFailed')),
        }));
      } finally {
        setFullLoading(null);
      }
  ...
  if (!apiReady || loading) {
    return (
      <Box sx={{ paddingTop: 2, paddingX: 2 }}>
        ...
        <Box sx={{ marginTop: 2 }}>
          <LoadingState>{t('chatbotManual.loading')}</LoadingState>
        </Box>
      </Box>
    );
  }
  ...
  if (!sorted.length) {
    return (
      <Box sx={{ paddingTop: 2, paddingX: 2 }}>
        <PageHeader title={title} subtitle={subtitle} />
        <Box sx={{ marginTop: 2 }}>
          <EmptyState>{t('chatbotManual.empty')}</EmptyState>
          <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
            {t('chatbotManual.inboxHint').replace('{path}', data?.inboxRelative ?? 'chatbot')}
          </Typography>
        </Box>
      </Box>
    );
  ...
  return (
    <Box sx={{ paddingTop: 2, paddingX: 2, paddingBottom: 3 }}>
      ...
      <Stack component="nav" spacing={1.25} sx={{ mt: 1 }} aria-label={t('chatbotManual.listAria')}>
        {sorted.map((row) => (
          <Paper
            key={row.fileName}
            elevation={0}
              ...
              >
                <Stack spacing={0.35} sx={{ flex: '1 1 auto', minWidth: 140 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {t('chatbotManual.updated')} {formatDate(new Date(row.mtimeMs ?? 0))}{' · '}
