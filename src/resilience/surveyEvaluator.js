...
async function assessBatch(batch, batchNum, totalBatches) {
...
function cpPath(date, sourceFile) {
  ...
  return `resilience/survey-checkpoint-${date}-${safe}.json`;
}
...
async function synthesizeRegional(munAssessments, date) {
  const componentList = RESILIENCE_COMPONENTS
    .map((c) => `- ${c.id}: ${c.name_en}`)
    .join('\n');
 ...
 */
export async function analyzeSurvey(municipalities, date, sourceFile) {
  ...
  if (!munAssessments) {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      const result = await withRetry(
        () => assessBatch(batches[i], i + 1, batches.length),
        `batch ${i + 1}`,
      );
  ...
  const regional = await withRetry(
    () => synthesizeRegional(munAssessments, date),
    'regional synthesis',
  );
