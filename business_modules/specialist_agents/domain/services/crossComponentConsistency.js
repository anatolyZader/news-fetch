...
function collectRefs(assessment) {
  ...
  for (const c of assessment.claims ?? []) {
    for (const r of c.evidence_refs ?? []) {
      // Critic-repair placeholder refs are not real evidence — a component whose
      // only refs are synthetic must not count as grounded for contradiction checks.
      if (String(r).startsWith('synthetic:')) continue;
      refs.add(String(r));
    }
  }
  return [...refs];
