...
export const DEFAULT_VALIDATION_CONFIG = Object.freeze({
  ...
  paths: {
    validation_dir: 'business_modules/resilience_scorer/validation/artifacts',
...
export function validationPaths(config, rootDir = process.cwd()) {
  ...
  const base = resolve(rootDir, p.validation_dir ?? 'business_modules/resilience_scorer/validation/artifacts');
  return {
    root: base,
...
export function computeElevationAdvisory(metrics, config) {
  ...
  return {
    ...
    message:
      ...
      `Consider setting operational_phase to "${suggested}" in business_modules/resilience_scorer/validation/validation-config.json.`,
  };
}
