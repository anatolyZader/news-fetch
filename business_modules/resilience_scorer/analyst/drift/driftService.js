...
function buildHistoryWindow(historyRaw, scope, days, endDate) {
  const effectiveEndDate = endDate ?? (historyRaw[historyRaw.length - 1]?.date ?? null);
  const dateWindow = effectiveEndDate && typeof effectiveEndDate === 'string'
...
function buildPerComponentSeries(history) {
  const per_component = {};
  for (const id of COMPONENT_IDS) {
    ...
    per_component[id] = { series: interpolateScoreGaps(seriesRaw) };
  }
  return per_component;
}

function dailyComponentMean(history, field) {
...
function buildDriftAlerts(history, daily_mean_polarization) {
...
export function createDriftService({ reportsDir, historyReader } = {}) {
  ...
  function compute({ scope = 'national', days = 30, endDate } = {}) {
    ...
    const history = buildHistoryWindow(historyRaw, scope, days, endDate);
    const dates = history.map((r) => r.date);
    const overall_series = interpolateScoreGaps(history.map((r) => ({ date: r.date, score: r.overall_score })));
    const per_component = buildPerComponentSeries(history);
    const daily_mean_polarization = dailyComponentMean(history, 'polarization');
    const daily_mean_certainty = dailyComponentMean(history, 'certainty');
    const daily_mean_erosion = dailyComponentMean(history, 'erosion_index');
    const daily_mean_chronic_z = dailyComponentMean(history, 'z_score_chronic');
    ...
    return {
      ...
      alerts: buildDriftAlerts(history, daily_mean_polarization),
      signal_volume_per_day: history.map((r) => ({
        date: r.date,
        total: Object.values(r.signal_counts).reduce((s, n) => s + n, 0),
        by_type: r.signal_counts,
      })),
      source_share_per_day: history.map((r) => ({
        date: r.date,
        total: Object.values(r.source_type_mass).reduce((s, n) => s + n, 0),
        by_source_type: r.source_type_mass,
      })),
    };
  }
