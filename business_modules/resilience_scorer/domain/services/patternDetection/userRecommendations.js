/**
 * Map semantic pattern alerts to user recommendations (HITL pending).
 *
 * Pipeline position: STAGE-2 assess finalize — converts deterministic pattern
 * detections into pending user recommendations on the assessment payload.
 *
 ...
 * `services/user/`, report finalize and client recommendation UI.
 */
...
/**
 * Map semantic pattern alerts to pending user recommendations.
 * @param {Array<object>} patterns from `detectSemanticPatterns`
 * @returns {Array<object>}
 */
export function buildUserRecommendations(patterns) {
