 ...
 * separate developer/redacted view.
 *
 * Key collaborators: `epistemic/thinEvidencePolicy.js`, `narrativeGrounding/groundingConfig.js`,
 * `contracts/displayViews.js`, `user/attentionItems.js`, report HTTP routes.
 */
...
export function deriveInstrumentState(comp, assessmentContext = {}) {
  ...
  const instrument = {
    ...
    shows_assessment: thinPolicy?.userShowsScore ?? sufficiency !== 'none',
    // Legacy key kept for UI compatibility; mirrors shows_assessment.
    user_shows_score: thinPolicy?.userShowsScore ?? sufficiency !== 'none',
 ...
 * @returns {string}
 */
export function userAssessmentSummary(assessment) {
 ...
 * @param {'user' | 'developer'} [_view] Kept for call-site compatibility.
 * @returns {object}
 */
export function redactAssessmentForView(assessment, _view) {
  ...
  const components = (assessment.components ?? []).map((c) => ({
    ...
    evidence: c.evidence_user_structured?.length
      ? c.evidence_user_structured
      : (c.evidence_user ?? c.evidence),
    instrument: deriveInstrumentState(c, assessmentContext),
  }));

  return {
    ...assessment,
    display_view: DISPLAY_VIEWS.user,
    components,
...
/**
 * Shape cached report payload for the single user report surface.
 *
 * @param {object} payload Cached report payload (assessment, markdown, …).
 * @param {'user' | 'developer'} [view] Kept for call-site compatibility.
 * @returns {object}
 */
