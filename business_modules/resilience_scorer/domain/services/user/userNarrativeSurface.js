/**
 * Finalize user-readable narrative and curated evidence on each component.
 *
 ...
 *
 * Owns: narrative_user resolution, citation linking, curated evidence bullets,
 ...
 * `contracts/citationDisplay.js`, `user/userInvestigationSurface.js`.
 */
...
import { userMaxClaims } from '../../contracts/userSurfaceMode.js';
import {
...
import { buildDeterministicNarrativeFromClaims } from './userInvestigationSurface.js';
...
function hasHybridPolishedNarrative(comp) {
  const existing = String(comp?.narrative_user ?? '').trim();
...
/**
 * Build user prose from claims (capped; rich mode uses role-section layout).
 *
 * @param {object[]} claims
 * @returns {string}
 */
...
function claimsFromRichPool(comp) {
  ...
  const highlights = comp.evidence_user_structured ?? [];
...
/**
 * Resolve inline signal citations and APA parentheticals in user narrative text.
 *
 * @param {string} text
 ...
 * @returns {string}
 */
export function resolveUserNarrativeCitations(text, registry, reportDate, componentId = null, comp = null) {
...
/**
 * Build qualitative user prose from epistemic slice when claims are absent.
 *
 * @param {string} componentId
 * @param {object} [ep={}] Count-based epistemic fields (signal_count, contested, thin).
 * @returns {string}
 */
export function buildEpistemicUserProse(componentId, ep = {}) {
...
/**
 * Build markdown evidence bullet strings for user display.
 *
 * @param {object} comp
 * @returns {string[]}
 */
export function buildCuratedEvidenceBullets(comp) {
  if (isRichSurfaceMode() && Array.isArray(comp.evidence_user_structured) && comp.evidence_user_structured.length > 0) {
    return comp.evidence_user_structured.map((item) => item.markdown).filter(Boolean);
  }

  if (Array.isArray(comp.evidence_user) && comp.evidence_user.length > 0) {
    return comp.evidence_user;
  }
...
/**
 * Build structured evidence items (text, ref, markdown) for user UI.
 *
 * @param {object} comp
...
export function buildStructuredEvidenceItems(comp) {
  if (Array.isArray(comp.evidence_user_structured) && comp.evidence_user_structured.length > 0) {
    return comp.evidence_user_structured;
  }
...
/**
 * Resolve final user narrative for one component (hybrid LLM vs deterministic).
 *
 * @param {object} comp
 * @returns {string|null}
 */
export function resolveUserComponentNarrative(comp) {
  const existing = String(comp.narrative_user ?? '').trim();
  ...
  if ((ep.signal_count ?? 0) > 0) {
    return buildEpistemicUserProse(comp.component_id, ep);
  }
 ...
 * @param {object} comp
 */
function finalizeUserComponentSurface(comp) {
  const hasRichPool = isRichSurfaceMode()
    && Array.isArray(comp.user_investigation_pool)
    && comp.user_investigation_pool.length > 0;

  if (!hasHybridPolishedNarrative(comp)) {
    const narrative = resolveUserComponentNarrative(comp);
    if (narrative) {
      comp.narrative_user = narrative;
    } else if (hasRichPool && isStubNarrative(comp.narrative_user)) {
      delete comp.narrative_user;
    }
  }

  const bullets = buildCuratedEvidenceBullets(comp);
  const structured = buildStructuredEvidenceItems(comp);
  if (bullets.length > 0 && comp.user_evidence_tier !== 'rich_pool') {
    comp.evidence_user = bullets;
    comp.evidence_user_structured = structured;
    comp.user_evidence_tier = 'curated';
  } else if (!hasRichPool && bullets.length === 0) {
    comp.user_evidence_tier = 'none';
  }
}
...
function applyCrossComponentSynthesisFallback(assessment) {
  if (assessment.cross_component_synthesis_user) return;
  const synthesis = String(assessment.cross_component_synthesis ?? '').trim();
  if (synthesis) {
    assessment.cross_component_synthesis_user = synthesis;
  }
}
...
function applyDegradedNarrativeCaveat(assessment) {
  ...
  const caveat =
    ...
    + 'prefer evidence_user bullets and user display states.\n\n';
...
function applyCitationRegistryToAssessment(assessment, citationRegistry) {
  ...
  if (typeof assessment.cross_component_synthesis_user === 'string') {
    assessment.cross_component_synthesis_user = applyCitationResolverToField(
  ...
  for (const comp of assessment.components ?? []) {
    if (typeof comp.narrative_user === 'string') {
      comp.narrative_user = applyCitationResolverToField(
        comp.narrative_user,
        citationRegistry,
        reportDate,
        comp.component_id,
...
/**
 * Finalize user narrative and evidence fields on all components (deterministic).
 *
 * @param {object|null|undefined} assessment
 * @returns {object|null|undefined}
 */
export function finalizeUserNarrativeSurface(assessment) {
  ...
  for (const comp of assessment.components ?? []) {
    ...
    finalizeUserComponentSurface(comp);
  }
