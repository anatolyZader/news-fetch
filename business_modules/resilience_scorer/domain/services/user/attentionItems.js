/**
 * Derive unified attention items for the user report panel.
 *
 ...
 *
 * Key collaborators: `user/assessmentDisplayTier.js`, `user/decisionBriefPrompt.js`,
 * assessment.data_void, pattern alerts, user_recommendations.
 */
 ...
 * user recommendations) are resolved via {@link kindForItem} fallback.
 */
const KIND_BY_CODE = Object.freeze({
  ...
  // Pipeline — measurement/catalog housekeeping (developer audience)
  ...
  assessment_empty: ATTENTION_KINDS.pipeline,
});
...
function kindForItem(it) {
  ...
  // Pattern detections and user recommendations carry an actionable channel set.
 ...
 * @param {{ view?: 'user' | 'developer', reportScopeId?: string, priorReports?: Array<object> }} [opts]
 * @returns {Array<object>}
 */
export function buildAttentionItems(assessment, opts = {}) {
  if (!assessment || typeof assessment !== 'object') return [];

  const view = opts.view === DISPLAY_VIEWS.developer
    ? DISPLAY_VIEWS.developer
    : DISPLAY_VIEWS.user;
  const isDeveloper = view === DISPLAY_VIEWS.developer;
  ...
  addOovAttentionItems(push, item, assessment, isDeveloper, methodology);
  addEmptyAssessmentAttentionItem(push, item, assessment);
  addUserRecommendationItems(push, item, assessment.user_recommendations);

  // Pattern detections carry channels/evidence; surface only those not already
  // represented by a pending user recommendation (recs are 1:1 with patterns).
  const coveredPatternCodes = new Set(
    (assessment.user_recommendations ?? [])
 ...
 *
 * This is a pipeline failure, not a quiet day: north 2026-04-12 shipped with
 * 782 signals, zero claims and all eight components `specialist_skipped`, and
 * nothing in the report said so. Guarded by a signal floor so genuinely thin
 * days (a handful of signals) stay silent.
 *
 * @param {Function} push
 * @param {Function} item
 * @param {object} assessment
 */
function addEmptyAssessmentAttentionItem(push, item, assessment) {
  const components = assessment.components ?? [];
  if (components.length === 0) return;

  const claimCount = components.reduce((n, c) => n + (c.narrative_claims?.length ?? 0), 0);
  if (claimCount > 0) return;

  // Only `scoped_signal_count` counts each signal once. Summing per-component
  // `signal_count` double-counts every signal that routes to several
  // components, which inflated a 23-signal day to 60 and fired this alert on a
  // genuinely quiet report. When the field is absent (older reports) there is
  // no trustworthy volume measure, so stay silent rather than cry wolf.
  const signalCount = Number(assessment.scoped_signal_count);
  if (!Number.isFinite(signalCount) || signalCount < EMPTY_ASSESSMENT_SIGNAL_FLOOR) return;

  const skipped = components.filter((c) => c.assessment_state === 'specialist_skipped').length;
  push(item('critical', 'pipeline:assessment_empty', 'assessment_empty', 'attention.pipeline.assessmentEmpty', {
    detail_key: 'attention.pipeline.assessmentEmptyDetail',
    detail_params: {
      signals: signalCount,
 ...
 * @param {object|null} methodology
 */
function addOovAttentionItems(push, item, assessment, isDeveloper, methodology) {
  ...
  if (isDeveloper && (assessment.oov_capture_count ?? 0) > 0) {
  ...
  if (isDeveloper && (methodology?.calibration?.deficit ?? 0) >= 0.5) {
 ...
 * data void. This prevents users from habituating to persistent grey/no-data states and
 ...
 */
function addUserRecommendationItems(push, item, recommendations) {
