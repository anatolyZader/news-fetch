/**
 * Prompt blocks and payload shaping for batch user decision brief (no re-scoring).
 *
 ...
 *
 * Key collaborators: `user/attentionItems.js`, `user/assessmentDisplayTier.js`,
...
function formatComponentInstrumentLine(comp) {
  ...
  const userSuffix = inst.user_status
...
/**
 * System prompt for user decision-brief JSON synthesis (no numeric scores).
 *
 * @returns {string}
 */
export function buildDecisionBriefSystemPrompt() {
  return (
    'You are a user decision-support assistant for Israeli homefront community resilience assessments.\n' +
    ...
    '- If assessment_mode is "abstained" or sampling_status is "blind", the summary must state that metrics are withheld and users should rely on field corroboration.\n' +
    '- Never contradict instrument abstention (insufficient_data, sampling_blind, limited_evidence_neutral).\n' +
    '- Do not invent new user_recommendation IDs; reference only recommendation_id values provided.\n' +
    '- suggested_next_step must be advisory (review, clarify, monitor) — never claim resources were dispatched.\n' +
    '- SPECIFICITY (required): every suggested_next_step must name (a) a place/scope — use the grounding.affected_clusters names or the report scope; (b) a counterpart — one of: field team, PBO/community coordinator, regional authority, or chat/developer; and (c) an observable success_signal stating how the user will know it worked.\n' +
    ...
    'OUTPUT JSON schema:\n' +
    '{\n' +
    '  "summary": "string (2-3 sentences, user-safe, no repetition)",\n' +
    '  "priority_items": [\n' +
    '    {\n' +
...
export function buildDecisionBriefUserPrompt(payload) {
  return (
    ...
    `Pending user recommendations:\n${JSON.stringify(payload.user_recommendations, null, 2)}\n\n` +
