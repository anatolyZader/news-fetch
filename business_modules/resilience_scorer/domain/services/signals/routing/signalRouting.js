/**
 * Signal → component routing weights, roles, and routing validation.
 *
 * Pipeline position: after the closed catalog. Extraction emits catalog types;
 * this map decides which resilience components each type contributes evidence to
 * (and with what signed prior). Assess uses these edges when collecting
 * per-component signal groups (componentEvidence / signalWeights).
 *
 * Owns: SIGNAL_TO_COMPONENTS priors, primary vs inferred edge roles,
 * coherence checks against the catalog and COMPONENT_IDS.
 *
 * Does NOT own: the type vocabulary itself (signalCatalog.js). Does not emit
 * numeric 1–10 component scores (min-math uses count-based evidence bands).
 *
 * Weight scale: directional ROUTING PRIORS in [-1.5, +1.5] — how strongly one
 * instance of a type bears on a component before intensity/grounding and other
 * instance factors. |w| ≈ 1 is a direct primary observation; |w| > 1 is
 * reserved for exceptionally diagnostic signals; |w| < 0.5 is usually a
 * secondary/inferred association (see SIGNAL_ROUTING_ROLES).
 *
 * Key collaborators: signalCatalog.js, componentEvidence.js / signalWeights.js,
 * signalRouter.js (facade), assessmentMethodology.js (version bump on edits).
 */
import {
  SIGNAL_CATALOG,
  SIGNAL_ALIASES,
...
/**
 * Canonical type → { componentId: signed routing prior }.
 * Every SIGNAL_CATALOG type must have an entry; aliases must not.
 * Prefer reading via signalRouter.getComponentWeight for canonicalize + lookup.
 * @type {Record<string, Record<string, number>>}
 */
export const SIGNAL_TO_COMPONENTS = {
  accountability_demand_constructive: {
    leadership: +0.7,
    information_communication: +0.3,
  },
  active_information_seeking: {
    information_communication: +0.7,
  },
  adaptive_practice: {
    functional_continuity: +0.8,
    community_capital: +0.5,
  },
  agricultural_damage: {
    functional_continuity: -0.6,
    community_capital: -0.3,
  },
  anniversary_distress_uptick: {
    wellbeing_at_risk: -0.7,
    narrative: -0.3,
  },
  blame_narrative: {
    narrative: -0.8,
    leadership: -0.3,
    belonging_solidarity: -0.3,
  },
  blame_shifting: {
    leadership: -0.9,
    narrative: -0.3,
  ...
  bridging_capital_demonstrated: {
    belonging_solidarity: +0.9,
    community_capital: +0.5,
  },
  bridging_capital_failure: {
    belonging_solidarity: -0.9,
    community_capital: -0.4,
  },
  calm_confidence: {
    narrative: +0.9,
    wellbeing_at_risk: +0.5,
  },
  child_distress: {
    wellbeing_at_risk: -0.9,
  },
  ...
  civil_society_mobilization: {
    leadership: +0.5,
    community_capital: +0.9,
  },
  commemoration_event_observed: {
    narrative: +0.5,
    belonging_solidarity: +0.6,
  },
  community_volunteering: {
    community_capital: +1,
    belonging_solidarity: +0.7,
    wellbeing_at_risk: +0.5,
  },
  compensation_blocked: {
    functional_continuity: -0.7,
    wellbeing_at_risk: -0.5,
    leadership: -0.3,
  },
  compensation_received: {
    functional_continuity: +0.7,
    wellbeing_at_risk: +0.4,
  },
  complacency_or_normalization: {
    lifesaving_behavior: -0.7,
    information_communication: -0.4,
    narrative: -0.3,
  },
  compliance_enter_shelter: {
    lifesaving_behavior: +1,
    leadership: +0.2,
    belonging_solidarity: +0.2,
  },
  compliance_follow_instructions: {
    lifesaving_behavior: +0.9,
    leadership: +0.3,
  },
  compliance_norm_enforcement: {
    lifesaving_behavior: +0.7,
    belonging_solidarity: +0.5,
    community_capital: +0.4,
  },
  compliance_partial: {
    lifesaving_behavior: -0.6,
    leadership: -0.2,
  },
  conflict_or_tension: {
    belonging_solidarity: -1.1,
    wellbeing_at_risk: -0.4,
  },
  conflict_resolution: {
    community_capital: +0.6,
    belonging_solidarity: +0.6,
    leadership: +0.3,
  },
  ...
  coordination_failure: {
    leadership: -1,
    community_capital: -0.6,
    functional_continuity: -0.5,
  },
  coordination_success: {
    leadership: +1,
    community_capital: +0.6,
    functional_continuity: +0.4,
  },
  cross_event_learning: {
    functional_continuity: +0.6,
    leadership: +0.5,
    lifesaving_behavior: +0.4,
  },
  cultural_continuity: {
    narrative: +0.6,
    belonging_solidarity: +0.6,
  ...
  cyber_attack_on_infrastructure: {
    information_communication: -0.9,
    functional_continuity: -0.6,
  },
  deepfake_misinformation: {
    information_communication: -1,
    narrative: -0.4,
    leadership: -0.3,
  },
  connectivity_outage: {
    functional_continuity: -0.9,
    information_communication: -0.7,
    wellbeing_at_risk: -0.3,
  },
  ...
  early_warning_system_failure: {
    lifesaving_behavior: -1,
    information_communication: -0.6,
    leadership: -0.4,
  },
  institutional_abandonment_perception: {
    narrative: -0.9,
    leadership: -0.5,
    belonging_solidarity: -0.4,
  },
  population_survey_finding: {
    wellbeing_at_risk: -0.8,
    narrative: -0.3,
  },
  ...
  diaspora_solidarity: {
    community_capital: +0.7,
    belonging_solidarity: +0.5,
    narrative: +0.3,
  },
  digital_mutual_aid: {
    community_capital: +0.8,
    belonging_solidarity: +0.5,
  },
  ...
  // 15c: raised past render gate; dropped noise wellbeing edge
  ecosystem_stress: {
    functional_continuity: -0.5,
  },
  ...
  // 15c: narrative edge raised past render gate
  ...
  information_confusion: {
    information_communication: -1,
    leadership: -0.4,
    lifesaving_behavior: -0.3,
  },
  information_effectiveness_gap: {
    information_communication: -1,
    lifesaving_behavior: -0.5,
  },
  ...
  novel_behavior_observed: {
    wellbeing_at_risk: -0.35,
    community_capital: -0.25,
    information_communication: -0.2,
  },
...
/**
 * Derived map: for each signal type, each component edge is 'primary' or
 * 'inferred'. Built once from SIGNAL_TO_COMPONENTS + ADDITIONAL_PRIMARY_EDGES.
 * Callers should use getRoutingRole rather than reading this directly.
...
/** Warn when catalog defaultPolarity has no matching-sign weight in the mapping. */
...
/** Error on unknown component ids, zero/non-finite weights, or |w| > MAX. */
...
/** ADDITIONAL_PRIMARY_EDGES must name real mappings; every type needs ≥1 primary. */
 ...
 * weight ranges, indicator_kind rules, and routing-role integrity.
