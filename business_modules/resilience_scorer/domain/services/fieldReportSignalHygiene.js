...
export function isTrivialFieldReportEvidence(text) {
  ...
  if (TRIVIAL_EVIDENCE_RE.test(trimmed)) return true;
  const afterMuni = trimmed.replace(/^\[[^\]]+\]\s*/, '').trim();
  if (TRIVIAL_EVIDENCE_RE.test(afterMuni)) return true;
  return false;
}

/**
 * @deprecated Use rewriteMisclassifiedSignalType from signalTypeHygiene.js
 * @param {string} signalType
 * @param {string} evidence
 * @returns {string}
 */
export function rewriteMisclassifiedFieldReportType(signalType, evidence) {
  return rewriteMisclassifiedSignalType(signalType, evidence);
}
