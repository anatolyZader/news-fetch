...
export function splitAudioFileIfNeeded(inputPath, safeMaxBytes = SAFE_MAX_BYTES) {
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'radio-audio-'));
...
/**
 * @param {object} opts
 * @param {import('../infrastructure/adapters/openaiTranscriptionAdapter.js').OpenaiTranscriptionAdapter} opts.adapter
 */
export class RadioIngestService {
  constructor({ adapter }) {
  ...
  async ingestToMarkdown(p) {
    ...
    try {
      ...
      if (isTemp && parts[0]?.includes('radio-audio-')) {
    ...
    const lines = [
      `# Radio broadcasts articles (${date})`,
      '',
      `For population-behavior / Home Front resilience analysis (transcribed from radio).`,
    ...
    grouped.forEach((a, i) => {
      ...
      lines.push('');
      lines.push(`- **URL:** (radio recording)`);
