...
pipeline_command() {
  ...
  echo "$command" | grep -qE 'business_modules/(resilience_scorer|pbo_report_muni|pool|whatsapp|social_media|news-sites|audio|open_observation_extraction)/' && return 0
