...
```
npm run pipeline:run -- --preset 8comp-7-north [--force] [--date YYYY-MM-DD] 2>> logs/pipeline-run-north-<target date>.log
...
```
npm run pipeline:run -- --preset 8comp-7-north --assess-only [--date YYYY-MM-DD] 2>> logs/pipeline-run-north-<target date>.log
```
