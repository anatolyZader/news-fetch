...
**Field:** field is **not** scoped to the 3-day window. Glob **all** `business_modules/visits/data/articles-field-reports-*.md` files (any date), re-extract each one, and assess will accumulate the full history automatically. Do **not** match by window date — if an MD exists, extract it. If no MDs exist for the window dates, check the full glob; missing April MDs when March MDs exist is a data/export gap (someone must produce the missing MD), not a skip signal. Assess includes all visit signal bundles on or before target date.
