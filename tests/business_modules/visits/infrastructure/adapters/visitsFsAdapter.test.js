...
test('visits fs adapter loads field signals from resilience_scorer closed-signals dir when missing from module dir', () => {
  const rootDir = mkdtempSync(join(tmpdir(), 'visits-'));
  const reportsDir = join(rootDir, 'reports');
  const moduleSignalsDir = join(rootDir, 'business_modules', 'visits', 'data', 'signals');
  const legacySignalsDir = join(rootDir, 'business_modules', 'resilience_scorer', 'data', 'signals');
  mkdirSync(reportsDir, { recursive: true });
