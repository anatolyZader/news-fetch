...
} from '../../../../business_modules/resilience_scorer/app/assessment/userNarrativePipeline.js';
...
describe('userNarrativePipeline', () => {
  let prevPipeline;
  let prevRag;
  ...
  it('applyUserNarrativeToAssessment sets narrative_user in hybrid mode', () => {
    process.env.RESILIENCE_NARRATIVE_PIPELINE = 'hybrid';
    const assessment = {
      date: '2026-04-03',
      components: [{
  ...
  it('persists narrative_claims and resolves [S#] citations in hybrid mode', () => {
    ...
    assert.doesNotMatch(assessment.components[0].narrative_user, /\[S\d+\]/);
    assert.doesNotMatch(assessment.cross_component_synthesis_user, /\[S\d+\]/);
    assert.ok(assessment.narrative_citation_registry?.entries?.length > 0);
  });

  it('backfills narrative_user when polish omits component narrative', () => {
    process.env.RESILIENCE_NARRATIVE_PIPELINE = 'hybrid';
    const signals = fixtures.scored_components.narrative.signals;
    const registry = buildSignalRefRegistry({ narrative: { signals } });
    const assessment = {
    ...
    assert.ok(assessment.components[0].narrative_user);
    assert.match(assessment.components[0].narrative_user, /sleep disruption/i);
  ...
  it('applyUserNarrativePipeline stamps agent mode and finalizes surface', async () => {
    ...
    assert.equal(assessment.components[0].narrative_user, 'Agent prose unchanged.');
  });

  it('applyUserNarrativePipeline accepts partition metadata without error', async () => {
    process.env.RESILIENCE_NARRATIVE_PIPELINE = 'agent';
    const assessment = {
      components: [{ component_id: 'narrative', narrative: 'Agent prose unchanged.' }],
    };
  ...
  it('falls back to claim-derived user narrative when hybrid pipeline throws', async (t) => {
    // Mock setTimeout so the LLM retry backoff (5s + 10s) does not run in real time.
    t.mock.timers.enable({ apis: ['setTimeout'] });
    const assessment = {
      components: [{
    ...
    assert.equal(assessment.components[0].narrative_user, 'Agent narrative.');
    assert.ok(assessment.components[0].evidence_user?.length > 0);
  });
});
