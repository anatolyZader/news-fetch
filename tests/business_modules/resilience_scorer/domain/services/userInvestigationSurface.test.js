...
} from '../../../../../business_modules/resilience_scorer/domain/contracts/userSurfaceMode.js';
import {
...
} from '../../../../../business_modules/resilience_scorer/domain/services/user/userInvestigationSurface.js';
import {
  comparePoolItems,
  ...
  routingLabelSuffix,
} from '../../../../../business_modules/resilience_scorer/domain/services/user/evidenceFormatting.js';
...
describe('userInvestigationSurface', () => {
  const fearSignal = {
  ...
  it('assignUserEpistemicRole labels context and scored', () => {
    const scoringSet = new Set([fearSignal]);
    const quarantineSet = new Set();
    assert.equal(assignUserEpistemicRole(fearSignal, scoringSet, quarantineSet), 'scored');
    assert.equal(assignUserEpistemicRole(macroSignal, scoringSet, quarantineSet), 'context_only');
  });
  ...
  it('buildComponentInvestigationPool keeps only strong catalog links per component', () => {
    ...
    assert.equal(pool[0].user_epistemic_role, 'scored');
  });
  ...
  it('attachRichUserSurface populates pool without raw narrative dump', () => {
    ...
    try {
      const assessment = {
        components: [{
          ...
          user_display_state: 'insufficient_data',
      ...
      assert.equal(assessment.user_surface_mode, 'rich');
      assert.equal(comp.user_surface_starved, false);
      assert.ok(comp.user_investigation_pool?.length >= 1);
      assert.equal(comp.narrative_user, undefined);
      assert.equal(comp.user_evidence_tier, 'rich_pool');
      finalizeUserNarrativeSurface(assessment);
      assert.ok(comp.narrative_user?.length > 0);
      assert.ok(!comp.narrative_user.includes('avg='));
      assert.ok(comp.user_investigation_pool?.length >= 1);
    } finally {
  ...
  it('attachRichInvestigationPool preserves hybrid polished narrative_user', () => {
    const prev = process.env.RESILIENCE_OPERATOR_SURFACE_MODE;
    process.env.RESILIENCE_OPERATOR_SURFACE_MODE = 'rich';
    try {
      const polished = (
      ...
      assert.equal(comp.narrative_user, polished);
      assert.ok(comp.user_investigation_pool?.length >= 1);
      finalizeUserNarrativeSurface({ components: [comp] });
      assert.equal(comp.narrative_user, polished);
      assert.ok(!isStubNarrative(comp.narrative_user));
    } finally {
      if (prev === undefined) delete process.env.RESILIENCE_OPERATOR_SURFACE_MODE;
      else process.env.RESILIENCE_OPERATOR_SURFACE_MODE = prev;
    }
  ...
  it('buildDeterministicNarrativeFromClaims groups by role', () => {
    const prose = buildDeterministicNarrativeFromClaims([
      { text: 'Scored line', user_epistemic_role: 'scored' },
      { text: 'Context line', user_epistemic_role: 'context_only' },
    ]);
...
describe('routing rationale on evidence items', () => {
  ...
  it('context-only pool items are visibly labeled in highlight markdown', () => {
    ...
    assert.equal(pool[0].user_epistemic_role, 'context_only');
    const items = buildHighlightedEvidenceFromPool(pool);
  ...
  it('routingLabelSuffix and comparePoolItems behave on edge cases', () => {
    ...
    assert.equal(
      routingLabelSuffix({
        ...
        user_epistemic_role: 'context_only', signal_provenance: 'narrative_national_context',
      }),
    ...
    assert.equal(
      routingLabelSuffix({
        ...
        user_epistemic_role: 'context_only', signal_provenance: 'regional_press_context',
      }),
      ' `x · primary · regional press context`',
    );
    assert.equal(
      routingLabelSuffix({ signal_type: 'x', routing_role: 'primary', user_epistemic_role: 'quarantined' }),
      ' `x · primary · quarantined`',
    );
    assert.equal(
      routingLabelSuffix({ signal_type: 'x', routing_role: 'primary', user_epistemic_role: 'scored' }),
      ' `x · primary`',
    );
