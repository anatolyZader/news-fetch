...
} from '../../../../business_modules/pbo_report/infrastructure/pboDistrictRegistry.js';
...
test('resolveLocalExcelPaths reads legacy north module root', () => {
  ...
  try {
    mkdirSync(join(root, 'business_modules/pbo_report'), { recursive: true });
    writeFileSync(join(root, 'business_modules/pbo_report/north_1_2.xlsx'), 'x', 'utf8');
...
test('listPboDistrictsForApi exposes local and regional configuration', () => {
  ...
  try {
    mkdirSync(join(root, 'business_modules/pbo_report/data/muni/south'), { recursive: true });
    writeFileSync(join(root, 'business_modules/pbo_report/data/muni/south/south_3_4.xlsx'), 'x', 'utf8');
