...
describe('pipelineStatusService', () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(rootDir, 'business_modules/resilience_scorer/data/signals'), { recursive: true });
    ...
    writeFileSync(join(rootDir, 'business_modules/resilience_scorer/data/signals/signals-news-2026-05-27.json'), JSON.stringify({
      date: '2026-05-27',
      total_articles: 10,
  ...
  it('reports missing when assess report absent', () => {
    rmSync(join(rootDir, 'business_modules/resilience_scorer/data/daily_reports/resilience-report-2026-05-27.json'));
    const svc = createPipelineStatusService({ rootDir, timezone: 'Asia/Jerusalem' });
    const status = svc.getStatus({ date: '2026-05-27', scope: 'national' });
