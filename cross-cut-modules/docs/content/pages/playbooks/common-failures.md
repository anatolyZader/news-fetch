...
3. `curl -sS -o /dev/null -w "%{http_code}\n" http://SERVER/api/docs/index` — is `cross-cut-modules/docs/content/pages/` deployed?
...
- **Check**: `cross-cut-modules/log/data/cost-log.jsonl` for outlier entries; compare today's input volume to recent "normal" days.
...
- **Fix**: ensure the deploy includes `cross-cut-modules/docs/content/pages/`. If stub template pages surface, the index filter isn't excluding `_`-prefixed files; update `utils/productDocs.js` to match the validator's exclusions and redeploy.
