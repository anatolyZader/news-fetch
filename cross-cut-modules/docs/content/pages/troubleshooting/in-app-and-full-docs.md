...
**Semantic search (suggested topics)** — requires the `docs` RAG index. Users run `npm run rag:reindex-docs` after product docs change (typically post-`docs:sync` on deploy). When `DOCS_RAG_ENABLED=0`, the panel falls back to title/tag filtering only.

## Something is broken in the product
For user-level recovery (auth, pipeline, ingestion), see [Common failures](../playbooks/common-failures.md) on full docs, or ask your deployment user.

## Troubleshooting
- **Open full docs does nothing** — your browser may block pop-ups; allow them for this site or copy the link from the footer.
- **Still stuck** — contact your user with the page you expected and what you see instead.
