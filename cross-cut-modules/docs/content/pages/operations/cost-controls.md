...
- **Recommended**: A habit of reviewing `cross-cut-modules/log/data/cost-log.jsonl` at the end of each run.
...
- **Cost log** — `cross-cut-modules/log/data/cost-log.jsonl`, appended to by each LLM call.
...
- Watch `cross-cut-modules/log/data/cost-log.jsonl` daily for outliers.
...
```bash runnable
ls -1 cross-cut-modules/log/data/cost-log.jsonl 2>/dev/null || true
```
...
```bash runnable
tail -n 500 cross-cut-modules/log/data/cost-log.jsonl 2>/dev/null | jq -s 'map(.cost_usd // 0) | add' 2>/dev/null || echo "jq not installed"
```
...
```bash runnable
tail -n 200 cross-cut-modules/log/data/cost-log.jsonl 2>/dev/null | sort -t'"' -k8 -n 2>/dev/null | tail -n 5 || true
```
...
- **Costs jumped day-over-day without an obvious cause**
  ...
  - **Fix**: compare `cross-cut-modules/log/data/cost-log.jsonl` between two days and look for a different input size or a new source tag. Reconcile one side to match.
...
- **Cost log is missing entries I expected**
  - **Check**: whether the script path actually writes to `cross-cut-modules/log/data/cost-log.jsonl` — older ad-hoc scripts may not.
