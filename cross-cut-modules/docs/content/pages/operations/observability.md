...
3. **Are docs served?** `curl /api/docs/index` → `200` with a non-empty array means `cross-cut-modules/docs/content/pages/` is deployed.
...
Expected: JSON with a `pages` array. Empty array or `404` means `cross-cut-modules/docs/content/pages/` wasn't deployed with the server.
...
- **Docs panel in the app is empty**
  ...
  - **Fix**: ensure the server deploy includes `cross-cut-modules/docs/content/pages/`, then restart the server.
