...
- **Docs panel in the app says "failed to load"**
  ...
  - **Fix**: if that works, the UI has a stale proxy — hard-reload the page. If it doesn't, server is not serving `cross-cut-modules/docs/content/pages/` correctly; see [Observability](../operations/observability.md).
